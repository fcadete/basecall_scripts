...
pdf("soft_clipped_sequence_analysis.pdf", width = 16, height = 12)

subtelomere_end_bams <- list.files("soft_clipped_ends/", pattern = "rhietman_mapont_primary_subtelomere_start.bam")

full_frame <- data.frame()

for (file in subtelomere_end_bams) {

    this_bam <- scanBam(paste0("soft_clipped_ends/", file))
