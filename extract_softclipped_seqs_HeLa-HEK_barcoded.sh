...
for f in HeLa-HEK_barcoded/terra_primer_separated/*/Joana_TERRA_VNP_no_PCR_on_rhietman_mapont_primary.bam
do
  bam_output_f=${f/primary.bam/primary_subtelomere_start.bam}
  fq_output_f=${f/primary.bam/primary_subtelomere_start_softclipped.fq.gz}
  srun -n1 -N1 shifter samtools view -L subtelomere_starts_1_5000.bed -b $f > $bam_output_f
  srun -n1 -N1 ~/SE-MEI/extractSoftclipped $bam_output_f > $fq_output_f
done
