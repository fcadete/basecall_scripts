...
pdf("soft_clipped_sequence_analysis_q30.pdf", width = 16, height = 12)

subtelomere_end_bams <- list.files("soft_clipped_ends_q30/", pattern = "rhietman_mapont_q30_subtelomere_start.bam")

full_frame <- data.frame()

for (file in subtelomere_end_bams) {

    this_bam <- scanBam(paste0("soft_clipped_ends_q30/", file))
